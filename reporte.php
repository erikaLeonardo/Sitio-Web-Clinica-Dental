...
$consulta = "SELECT agendar.idAgendar AS NUM,  
date_format(horarios.fecha, '%W %e de %M %Y') AS FECHA,
CONCAT(nombrePaciente, ' ', apellidosPaciente) AS PACIENTE,
horas.hora AS HORA,
motivo.nombreMotivo AS TRATAMIENTO
FROM
agendar
INNER JOIN
pacientes
ON
agendar.idPaciente = pacientes.idPaciente
INNER JOIN
consulta
ON
agendar.numeroConsulta = consulta.numeroConsulta
INNER JOIN
motivo
ON
agendar.idMotivo = motivo.idMotivo
INNER JOIN
horarios
ON
...
WHERE agendar.estatus = '1'
ORDER BY agendar.idAgendar ASC";
