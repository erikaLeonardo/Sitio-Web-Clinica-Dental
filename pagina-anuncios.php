...
include("conexion/conexion.php");
    ...
    if(isset($_SESSION['nombreU']) && isset($_SESSION['nombreUsuario']) && isset($_SESSION['edadusuario']) && isset($_SESSION['usuario']) && isset($_SESSION ['celularusuario'])){
        ...
        echo "<section class='row'>";
            echo "<article class='col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 ps-5'>";
                echo "<h2 class='bodyTitle fs-1'>Anuncios</h2>";
            echo "</article>";
            echo "<article class='col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 ps-5'>";
                echo "<h4 class='body'>Anuncio Actual</h4>";
            echo "</article>";
        echo "</section>";
        echo "<section class='row ps-5 pe-5'>";
            echo "<div class='col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12' style='background-color: #d3d3d3'>";
            // echo "<div class='col-12 col-sm-12 col-md-12 ms-5 me-5>";
                echo "<br>";
                $num = 3;
                ...
                while($actual = mysqli_fetch_array($anuncios)){
                    echo "<table class='col-12 col-sm-12 col-md-12 col-lg-12 col-xxl-12' style='background-color: #d3d3d3'>";
                    echo "<tr>";
            ...
            echo "<br>";
            echo "<br>";
            echo "<div class='float-end pb-5'>";
            echo "<br>";
            echo "<a class='btn btn-pcolor float-end' href='pagina-anuncios.php?e=".$num."'>Eliminar anuncio</a>";
            echo "<a class='btn btn-pcolor float-end me-2' href='anuncios-nuevos.php'>Agregar anuncio</a>";
            // echo "<button class='btn float-end ms-5 me-5' style='background-color: #59C1BD'>Eliminar anuncio</button>";
            // echo "<button class='btn float-end ms-5' style='background-color: #59C1BD' href='anuncios-nuevos.php' >Agregar anuncio</button>";
            echo "</div>";
        echo "</section>";
    echo "</main>";
...
if(isset($_GET['e'])){
    ...
    $estatus = "SELECT * FROM anuncios";
    $cal = mysqli_query($conectar, $consulta);
    $este = mysqli_fetch_array($cal);

    if($este[2] == 1){

        $eliminarA = "CALL eliminarAnuncio('$eliminar')";
        if(mysqli_query($conectar, $eliminarA)){
            //header("location:crear-cuenta.php");
            header("location:pagina-anuncios.php");

        }else{
            echo "<script language='JavaScript'>alert ('Problemas al eliminar anuncio, intentalo mas tarde'); </script>";
        }
        mysqli_close($conectar);

    }else{
        echo "<script language='JavaScript'>alert ('No hay Anuncios para eliminar'); </script>";  
    }
}
?>
